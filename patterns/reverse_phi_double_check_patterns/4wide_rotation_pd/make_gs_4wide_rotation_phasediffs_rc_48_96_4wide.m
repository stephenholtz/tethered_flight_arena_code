...
flicker_offsets = [0 1 2 3 4 6];

pattern.x_num = 48*2; 	% 12 frames before repeat 
...
for j = 1:length(flicker_offsets)
    j
    flicks = [];unflicks = [];
    for ko = [12 12+24 12+24*2 12+24*3]
    flick = ko-flicker_offsets(j):ko+11-flicker_offsets(j);
    flicks = [flicks flick];
    end
...
name =  'Pattern_003_11_phasediffs_96_4wide_before_rotation';
