...
for i = 1:numel(genotypes)
    ...
    if ~sum(strcmpi(genotypes(i).name,{'.','..','.DS_STORE','thumbs'}))
        ...
        if ~sum(strfind([experiment_files.name],'processed_data'))
            ...
            for k = 1:numel(experiment_files)
                if ~sum(strcmpi(experiment_files(k).name,{'.','..','.DS_STORE','thumbs'}))
                    load(fullfile(cd,genotypes(i).name,experiment_files(k).name,'metadata.mat'))
                    disp(metadata.Arena)

                    if strfind(metadata.Arena,'WW23')
                       rmdir(fullfile(cd,genotypes(i).name,experiment_files(k).name),'s')
                       disp('KILLED IT!')
                    end
