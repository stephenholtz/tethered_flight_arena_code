...
if 01
    ...
    for i = 1:numel(geno_names)
        ...
        for condition_set_number = 1:12
            ...
            [avg_ts, ~]           = geno_data{i}.get_trial_data_set(condition_numbers,'x_pos','mean','yes','all',1);
            ...
            if sum(strcmpi(curve_name,{'ON_ON','ON_OFF','OFF_ON','OFF_OFF'}))
                ...
                for sub_stim = 1:numel(lmr_ts)
                    for trace = 1:size(lmr_ts{sub_stim},1)
                        ...
                        elseif shift_amt == 0
                            trace_iter = trace_iter + 1;
                            tuning_curves.raw.(curve_name)(i).ts{sub_stim}(trace_iter,:)            = lmr_ts{sub_stim}(trace,1:(1+end-buffer_amt));
                            tuning_curves.raw.(curve_name)(i).x_pos_ts{sub_stim}(trace_iter,:)      = x_pos_ts{sub_stim}(trace,1:(1+end-buffer_amt));
                            tuning_curves.raw.(curve_name)(i).voltage_ts{sub_stim}(trace_iter,:)    = volt_ts{sub_stim}(trace,1:(1+end-buffer_amt));
                        end
                    end
                end
...
if 1
    ...
    for cn =  {'on_off_edges_rear','on_off_edges_center','on_sweep','off_sweep','on_expansion','off_expansion','on_sawtooth','off_sawtooth'}
        ...
        for row = 1:2
            ...
            tfPlot.timeseries(graph);
            plot((tuning_curves.(curve_name)(i).all_x_avg_ts{row})'/10)
            end
            box off
            %axis([0  -3.5 3.5])
