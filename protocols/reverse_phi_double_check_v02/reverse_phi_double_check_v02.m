function Conditions = reverse_phi_double_check_v02
...
% gather some information
    cf = pwd;
    patterns = what(fullfile(dir,'patterns','reverse_phi_double_check_patterns_v02'));
    pattern_loc = patterns.path;
    patterns = patterns.mat;
    pos_func_loc = fullfile(dir,'position_functions','reverse_phi_double_check_v02_functions');
...
cond_num = 1;
total_ol_dur = 0;
frequency = 200; % This is so it can = the fps from gain + 2.5bias
duration = 2;
...
if tuthillesqe
for pat = 2:83 % Hard coded phase delays with each pattern (coded in the name of the pattern completely)
    ...
    % The direction is determined by the pattern...
...
% Make my stimuli:
if holtzian
for pat = 1; % 4 wide full field
    % the different temporal frequency position functions
    for pos_funcX = [1 2  24 25 39 40] % [.75cw .75ccw 2cw 2ccw 4cw 4ccw]
