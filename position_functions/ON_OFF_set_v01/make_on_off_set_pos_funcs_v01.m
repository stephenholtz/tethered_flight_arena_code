...
for dps_wanted = dps_set
    ...
    actual_dps = 1000*(ms_per_sample * actual_samples_per_step * (1/degrees_per_step))^-1;
    ...
    for pos = 2:97
        ...
        during = [during pos*ones(1,real_num_steps)];

    end

    after = ones(1,time_after/ms_per_sample);

    stim_time = 1000*(numel(during) + numel(before + numel(after)));
