function Conditions = unilat_rev_phi_phase_delay_4_wide_v01
...
for pat = [14 20]; % [4 Wide left-side 4 wide right-side]
    % the different temporal frequency position functions
    for pos_funcX = [1 2 20 21 39 40]; % [.5cc .5cw 2.27cc 2.27ccw 4.16cw 4.16ccw]
        ...
        for flick = 1:2; % for both before and after the movement
            if flick == 1;
                switch pos_funcX
                    ...
                    case {20, 21} % tf 2.27
                        delay_funcs_y = [22 23:2:38]; % [0]
                        delay_funcs_y = fliplr(delay_funcs_y);
                    case {39, 40} % tf 4.16
                        delay_funcs_y = [41 42:2:53]; % [0]
                        delay_funcs_y = fliplr(delay_funcs_y);
                end
            else
                switch pos_funcX
                    ...
                    case {1, 2} % tf .5
                        delay_funcs_y = 5:2:19;
                    case {20, 21} % tf 2.27
                        delay_funcs_y = 24:2:38;
                    case {39, 40} % tf 4.16
