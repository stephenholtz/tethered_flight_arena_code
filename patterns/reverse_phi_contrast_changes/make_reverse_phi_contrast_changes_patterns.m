...
project = 'reverse_phi_contrast_changes';
counter = 0;
...
for stim_type = 1:4
    ...
    for size = [2 4 6 8 12 24]
            pattern_size_name = [name, num2str(size), '_wide'];
        
        for contrast = [1 2 3 4 5 6];
            pattern_name = [pattern_size_name  '_gs3_bar_', num2str(contrast)];
            ...
            if sum(stim_type == [1 2 3 4])
                    ...
                    Pats(:, :, 1, 1) = repmat([contrast*ones(4,size), 6*ones(4,size)],1,96/(size*2));
                    Pats(:, :, 1, 2) = repmat([contrast*ones(4,size), 0*ones(4,size)],1,96/(size*2));
