...
if 0
    ...
    for i = 1:2
        ...
        for condition_set_number = 1:numel(curve_names)
            ...
            lr_summ_data.names{condition_set_number} = curve_name;
            
            % add an example space time diagram to the lr_summ_data
            if 1
                ...
                lr_summ_data.(curve_name).space_time_diagram = frame_info.cdata;
                clear std_hand fig_hand frame_info
                close all
            end
            
            lr_summ_data.(curve_name).raw(i).grouped_conditions = geno_data{1}.grouped_conditions(condition_set_number);
            ...
            lr_summ_data.(curve_name).raw(i).avg_lmr = a;
            lr_summ_data.(curve_name).raw(i).sem_lmr = v;
            
            [a, v] = geno_data{1}.get_trial_data_set(condition_numbers,'lmr','none','no','all',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).avg_lmr_ts = a;
            lr_summ_data.(curve_name).raw(i).sem_lmr_ts = v;
            
            [a, v] = geno_data{1}.get_trial_data_set(condition_numbers,'lmr','none','no','exp',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).avg_per_fly_lmr_ts = a;
            lr_summ_data.(curve_name).raw(i).sem_per_fly_lmr_ts = v;
            
            [a, ~] = geno_data{1}.get_trial_data_set(condition_numbers,'lmr','none','no','none',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).all_lmr_ts = a;
            
            [a, v] = geno_data{1}.get_trial_data_set(condition_numbers,'x_pos','none','no','all',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).avg_x_pos_ts = a;
            lr_summ_data.(curve_name).raw(i).sem_x_pos_ts = v;
            
            [a, v] = geno_data{1}.get_trial_data_set(condition_numbers,'x_pos','none','no','exp',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).avg_per_fly_x_pos_ts = a;
            lr_summ_data.(curve_name).raw(i).sem_per_fly_x_pos_ts = v;
            
            [a, ~] = geno_data{1}.get_trial_data_set(condition_numbers,'x_pos','none','no','none',geno_norm_values(1));
            
            lr_summ_data.(curve_name).raw(i).all_x_pos_ts = a;
            
            lr_summ_data.note = 'The x_pos data is spared the direction conserving flip+average segment during analysis. In these stimuli, the patterns determine the CW vs CCW and the position functions are always the same for processing ease (at least I thought!).';
            
        end
    end
    
    save(fullfile(data_location,'lr_summ_data'),'lr_summ_data')
...
if ~exist('lr_summ_data','var')
    load(fullfile(data_location,'lr_summ_data'));
end
...
if steady_state_figure
        ...
        for col = 1:nWide
            ...
            if col == 1
                ...
                if make_stds
                    ...
                    subimage(lr_summ_data.(curve_name).space_time_diagram)
                    title('Space-Time')
                    axis off   
                end             
            end
    ...
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'steady_state_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'steady_state_no_flip_avg_v06'),'-fig'); end
...
if edge_sweep_figure
    ...
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'edges_sweep_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'edges_sweep_no_flip_avg_v06'),'-fig'); end
end
...
if competing_edges
    ...
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'competing_edges_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'competing_edges_no_flip_avg_v06'),'-fig'); end
end
...
if tuthill_opposed
    ...
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'tuthill_competing_edges_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'tuthill_competing_edges_no_flip_avg_v06'),'-fig'); end
end
...
if tuthill_motion
    ...
    lh=legend('x_pos','cw','ccw'); set(lh,'interpreter','none');
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'tuthill_on_off_motion_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'tuthill_on_off_motion_no_flip_avg_v06'),'-fig'); end
end
...
if rotation_30_lam
    ...
    if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'rotation_lam_30_no_flip_avg_v06'),'-pdf'); end
    %if save_figures; export_fig(tuningFigHandle(1),fullfile(data_location,'tuthill_on_off_motion_no_flip_avg_v06'),'-fig'); end
end