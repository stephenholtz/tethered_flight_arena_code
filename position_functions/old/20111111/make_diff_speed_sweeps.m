...
for sweepstart = 1:11   % 12 Positions
    ...
    func = init+addy; func = [func repmat(func(numel(func)),1,40)]; %#ok<*AGROW>
    
   count = count +1;
   if dir == 1; dirname = 'CW'; else dirname = 'CCW';end
   if count < 10; num = [ '0' num2str(count)] ; else num = num2str(count); end
   name = ['position_function_' num '_single_4px_movement_100hz_' dirname '_position_' num2str(sweepstart)];
