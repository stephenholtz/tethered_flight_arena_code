...
data_location = '/Users/stephenholtz/local_experiment_copies/ON_OFF_set_v03';
exp_dir = data_location;
geno_names{1} = 'gmr_53g02ad_gal80ts_kir21';
            ...
            if sum(strcmpi(curve_name,{'ON_ON','ON_OFF','OFF_ON','OFF_OFF'}))
                ...
                for sub_stim = 1:size(tuning_curves.pruned.(curve_name)(i).ts,2)
                    for trace = 1:size(tuning_curves.pruned.(curve_name)(i).ts{sub_stim},1)
                        tuning_curves.butter.(curve_name)(i).ts{sub_stim}(trace,:) = filter(B,A,tuning_curves.pruned.(curve_name)(i).ts{sub_stim}(trace,:));
                    end
                end
...
if 0
    ...
    tuningFigHandle(1) = figure( 'name' ,'Minimal Motion TS Figure','NumberTitle','off',...
        ...
        for row = 1:5
            ...
            hold on
            plot((((tuning_curves.pruned.(curve_name)(i).x_pos_ts{row})/14)-.01)')
        end
    end 
end
