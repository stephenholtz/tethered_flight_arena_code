function make_unilat_rev_phi_phase_delay_4_wide_v01_position_functions
%% info
...
project = 'unilat_rev_phi_phase_delay_4_wide_v01'; % Determines the destination show up
temporal_freqs = [.5 2.25 4];  % Set of tf's to use, the .5 is set below to show up as 0pt5 when saved
...
% THIS WILL BREAK (SEE LINE 165?) IF IT IS MORE THAN A FLICKER RECODE LATER
% FOR MORE GENERALITY!
pattern_y_length = 2;           % on and off flicker for the rev phi
max_num_delays = 8;% -1;        % if -1 it will make all, and check that the completely out of phase position function works...
...
polarities = 1;                 % Test both on and off motion with polarities = 2, just on with 1.
%% Make the functions
...
for temp_freq = temporal_freqs
    ...
    for chan = 1:2
        if chan == 1
            ...
            % Do this now for two polarities
            for pol = 1:polarities
                % no delay first
                switch pol
                    case 1
                        pol_type = '_off_motion';
                    case 2
                        pol_type = '_on_motion';
                end
                
                delay_name = [chan_name, pol_type, '_0ms_after_' num2str(sampling_rate) 'Hz'];
                ...
                for delay = delays
                    ...
                    for delay_type = [-1 1]
                        ...
                        switch delay_type
                            case 1
                                function_name = [chan_name, pol_type, '_', delay_val, 'ms_after_' num2str(sampling_rate) 'Hz'];
                                % SHIFT IT TO HAPPEN LATER
                                start_val = ~funct(1); % this is the equivalent of circshifting...
                                func = [repmat(start_val,1,(delay)) funct]; % I will never know why repmat fails here...
                                if mod(1,pol)
                                    func = ~func;
                                end
                                counter = counter + 1;
                            ...
                            case -1
                                function_name = [chan_name,pol_type, '_', delay_val, 'ms_before_' num2str(sampling_rate) 'Hz'];
