function [Conditions conds_matrix] = TESTING_conditions_small_field_motion_debugging
...
for i = 1:2 % 3 Posfunc/Pattern Combos
...
%         Conditions(cond_num).PosFunctionY = [2 0];
        ...
        if      i == 1; Conditions(cond_num).Duration = 36.3;
                        Conditions(cond_num).PosFunctionX = [1 2];
                        Conditions(cond_num).InitialPosition = [1 1];
                        Conditions(cond_num).PosFunctionY = [2 1];
                        Conditions(cond_num).FuncFreqX = 500;
        elseif  i == 2; Conditions(cond_num).Duration = 36.3;
...
%                         Conditions(cond_num).InitialPosition = [1 1];
%         elseif  i == 4; Conditions(cond_num).Duration = 21.3;
...
Conditions(q).RewardConditions.RewardDuration = .5;
