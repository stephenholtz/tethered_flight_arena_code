    ...
    for cell_type_group = 1:numel(cell_type_groups)
        ...
        for line = 1:numel(cell_type_groups(cell_type_group).stable_line_names)
            ...
            if 1
                ...
                % The genotype
                graph.avg{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).small_field.avg;
                graph.variance{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).small_field.sem;
            ...
            if 1
                ...
                % The genotype
                % The genotype
                graph.avg{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).small_field_donb.avg;
                graph.variance{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).small_field_donb.sem;
                graph.color{1} = mycolormap{1};
                % The average response
            ...
            if 1
                ...
                % The genotype
                graph.avg{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).osc_stim_ts.avg{1};
                graph.variance{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).osc_stim_ts.sem{1};
                graph.color{1} = mycolormap{1};
                % The average response
                graph.avg{2} = fig_data_summ(1).avg_tuning_curves.(normalization_type{1}).osc_stim_ts.avg(1:5000);
                graph.variance{2} = fig_data_summ(1).avg_tuning_curves.(normalization_type{1}).osc_stim_ts.sem(1:5000);
                graph.color{2} = mycolormap{2};
            ...
            if 1
                ...
                % The genotype
                graph.avg{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).osc_stim_ts.avg{2};
                graph.variance{1} = fig_data_summ(curr_line).tuning_curves.(normalization_type{1}).osc_stim_ts.sem{2};
                graph.color{1} = mycolormap{1};
                % The average response
                graph.avg{2} = fig_data_summ(1).avg_tuning_curves.(normalization_type{1}).osc_stim_ts.avg(5001:10000);
                graph.variance{2} = fig_data_summ(1).avg_tuning_curves.(normalization_type{1}).osc_stim_ts.sem(5001:10000);
                graph.color{2} = mycolormap{2};
            ...
            annotation('textbox',[0.01 line_y_pos(2) image_width/2 image_height],...
                'string',{'Dark','Stripe','Track'},'edgecolor','none','Fontsize',14)

            annotation('textbox',[0.01 line_y_pos(3) image_width/2 image_height],...
                'string',{'Bright','Stripe','Track'},'edgecolor','none','Fontsize',14)                    

            annotation('textbox',[0.01 line_y_pos(4) image_width/2 image_height],...
                'string',{'Grating','Track'},'edgecolor','none','Fontsize',14)  

            annotation('textbox',[0.01 line_y_pos(5) image_width/2 image_height],...
                'string',{'Track','Large-','bar','Offset'},'edgecolor','none','Fontsize',14)

            annotation('textbox',[0.01 line_y_pos(6) image_width/2 image_height],...
                'string',{'Track','Small-','bar','Offset'},'edgecolor','none','Fontsize',14)        
        ...
        if 1
            ...
            export_fig(gcf,fullfile(figure_location,[normalization_type{1} '_summary_figure_' figure_description{1}]),'-pdf')
            close all
