...
for temp_freq = temp_freq_list
    ...
    for bar_size = bar_size_list
        
        for direction = [-1 1]
            
            for stim_type = {'grat','edge'} 
                
                fps = 2*bar_size*temp_freq;
                
                switch stim_type{1}
                    
                    case 'grat'

                        [func,pos_func_samp_freq] = make_determine_best_simple_looping_position_functions(2*bar_size,temp_freq,direction,dummy_frame_flag);
                ...
                func_name = [stim_type{1} '_barsize_' num2str(bar_size) '_tf_' temp_freq_name 'Hz_fps_' num2str(fps) '_SAMP_RATE_' pos_func_samp_freq_name '_dir_' dir];
